...
| **Custom SVG chart kit** | The chart spec (docs/10 §6: exact mark widths, spacers, crosshair, table twins) is easier to guarantee in ~300 lines of owned SVG than by fighting a library's defaults; zero bundle cost. Recharts can be adopted later if standard forms outgrow the kit | Recharts — heavier and needs per-chart overrides to meet the spec |
